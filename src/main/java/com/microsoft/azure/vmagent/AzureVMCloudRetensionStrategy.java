...
public class AzureVMCloudRetensionStrategy extends RetentionStrategy<AzureVMComputer> {
    ...
    public AzureVMCloudRetensionStrategy(final int idleTerminationMinutes) {
     ...
     */
    @Override
    public final long check(final AzureVMComputer agentNode) {
        return check(agentNode, new ExecutionEngine());
    }

    private static final int CLEAN_VM_MAX_RETRIES = 30;
    private static final int CLEAN_VM_BACKOFF = 30;
    private static final int CLEAN_VM_TIMEOUT_IN_SECONDS = 30 * 60;

    protected final long check(final AzureVMComputer agentNode, final ExecutionEngine executionEngine) {
        ...
        if (canRecycle) {
            String actionStr = "shutdown";
            if (!agent.isShutdownOnIdle()) {
                actionStr = "delete";
            }
            LOGGER.log(Level.INFO, "AzureVMCloudRetensionStrategy: check: "
                            + "Idle timeout reached for agent: {0}, action: {1}",
                    new Object[]{agentNode.getName(), actionStr});
            ...
            try {
                executionEngine.executeAsync(task,
                        new LinearRetryForAllExceptions(
                                CLEAN_VM_MAX_RETRIES, // maxRetries
                                CLEAN_VM_BACKOFF, // waitinterval
                                CLEAN_VM_TIMEOUT_IN_SECONDS // timeout
                        ));
    ...
    @Override
    public final void start(final AzureVMComputer azureComputer) {
